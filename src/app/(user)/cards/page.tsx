    ...
    <div className={css({ minH: "100vh", bg: "#f4f4f4" })}>
      <div className="sber-container">
        <header className={stack({ gap: "4px", mb: "32px" })}>
          ...
          <h1 className={css({ fontSize: "24px", fontWeight: "800", color: "#000" })}>Мои карты</h1>
        </header>
                ...
                myCards.map(card => {
                  ...
                  return (
                    <a key={card.id} href={`/cards/${card.id}`} className="sber-card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '16px', textDecoration: 'none', transition: 'all 0.2s' }}>
                      <div className={css({ w: "52px", h: "34px", bg: "#f8fafc", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid", borderColor: "#f1f5f9", overflow: "hidden" })}>
                        {bankIcon ? (
                      ...
                      <div className={stack({ gap: "0", flex: "1" })}>
                        <p className={css({ fontWeight: "700", fontSize: "16px", color: "#000" })}>{card.cardName}</p>

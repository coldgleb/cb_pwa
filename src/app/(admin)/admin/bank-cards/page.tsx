...
export default async function AdminBankCardsPage() {
  ...
  return (
    <div className={stack({ gap: "32px" })}>
      <h1 className={css({ fontSize: "24px", fontWeight: "800", color: "#000" })}>Типы карт</h1>
      ...
      <section className="sber-card">
        <div className={flex({ align: "center", gap: "10px", mb: "24px" })}>
          ...
          <h2 className={css({ fontSize: "17px", fontWeight: "700", color: "#000" })}>Добавить тип карты</h2>
        </div>
          ...
          {allBankCards.map(card => {
            ...
            return (
              <a key={card.id} href={`/admin/bank-cards/${card.id}`} className="sber-card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div className={css({ w: "48px", h: "48px", bg: "#f8fafc", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid", borderColor: "#f1f5f9", overflow: "hidden" })}>
                  {bankIcon ? (
                ...
                <div className={stack({ gap: "0", flex: "1" })}>
                  <p className={css({ fontWeight: "700", fontSize: "15px", color: "#000" })}>{card.name}</p>
                  <p className={css({ fontSize: "12px", color: "secondaryText" })}>{card.bankName}</p>
                </div>

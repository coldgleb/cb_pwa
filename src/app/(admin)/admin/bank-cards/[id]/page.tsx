import { db } from "@/db";
import { banks, bankCards, bankCategories, bankCategoryMcc, bankCardSettings, merchants, bankCategoryMerchant } from "@/db/schema";
import { updateBankCard } from "@/lib/actions/bank-cards";
import { createBankCategory, updateBankCategory } from "@/lib/actions/categories";
...
import { ArrowLeft, ShieldCheck, Ban, Tag, Plus, Save, Trash2, History as HistoryIcon, Archive, Store } from "lucide-react";
              ...
              <div key={cat.id} className="sber-card" style={{ padding: '16px', border: isSystem ? '1px solid #e2e8f0' : (isArchived ? '1px dashed #cbd5e1' : undefined), background: isSystem ? '#f8fafc' : (isArchived ? '#f1f5f9' : 'white'), opacity: isArchived ? 0.7 : 1 }}>
                <form action={updateBankCategory.bind(null, cat.id)} className={stack({ gap: "16px" })}>
                  ...
                  <div className={flex({ gap: "8px", mt: "12px" })}>
                    {!isAllPurchases && (
                      <a 
                        href={`/admin/categories/${cat.id}/composition`}
                    ...
                    {isAllPurchases && (
                      <div className={css({ textAlign: "center", py: "8px", flex: 1, fontSize: "11px", fontWeight: "600", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" })}>
                        Действует на всё
                      </div>
                    )}
                  </div>
